...
一个 JSON-first 的参数化建筑确定性内核。持久化 JSON 是唯一几何真相源；
Schema 校验结果、Shapely 对象和 SVG 都是只读派生物，不会反向修改模型。

当前定位：**Production-oriented AI Agent Framework Prototype with constraint-aware
detached planning and evaluation**。它已完成当前 P1 安全硬化和 Phase 7 Task 7.1–7.2，但仍不是可直接
暴露到公网的 production-ready 服务。
...
本版本不连接任何真实 LLM/API，也不包含 Multi-Agent、自动修正、Three.js、
DXF、IFC 或规范 RAG。新增的 `MockLLMProvider` 只用于确定性测试和边界验收。
...
存在任何 error issue 时，应用服务和 `/render/svg` 都拒绝渲染并返回同一报告结构。
...
src/ai_parametric_architect/
  ...
  domain/            revisions, patches, audit, issues and precision policy
  ...
  renderer/          deterministic SVG adapter
